...
public class MailService {
	...
	// 인증 코드 검사하는 로직
	@Transactional
	public boolean verifyCode(MailReqBody mailReqBody) {

		Optional<Mail> mailOptional = mailRepository.findByEmail(mailReqBody.email());
		...
		if (mailOptional.isPresent()) {
			...
			if (!mail.getAuthCode().equals(mailReqBody.code())) {
